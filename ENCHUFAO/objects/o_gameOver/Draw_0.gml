...
if (alarm[0] <= 0)
{
	...
	draw_set_alpha(min(image_alpha,0.9))
	...
	draw_sprite_ext(sprite_index, 0, __view_get( e__VW.XView, 0 )+320, __view_get( e__VW.YView, 0 )+50, 1,1,0,image_blend, image_alpha);
	...
	if (image_alpha >=1)
	{
		...
		infoGameOver(info);
		draw_set_color(global.lightBlue)
		draw_text(__view_get( e__VW.XView, 0 )+319,__view_get( e__VW.YView, 0 )+331, "- " + string(text) + " -")
		draw_set_color(c_white)
		draw_text(__view_get( e__VW.XView, 0 )+320,__view_get( e__VW.YView, 0 )+330,  "- " + string(text) + " -")
