...
with(o_strandedShip)
{
	if (!chargedInHp) || (!chargedInEnergy)
	{
		...
		if (contLight <= 10)
		{
			draw_sprite_ext(s_lightBulb4, 0, x+lengthdir_x(5, _angle+90), y+lengthdir_y(5, _angle+90), 0.08+off, 0.08+off, 0, global.lightBlue, 0.5 + off);
		}
		else
		{
			draw_sprite_ext(s_lightBulb4, 0, x+lengthdir_x(5, _angle-90), y+lengthdir_y(5, _angle-90), 0.08+off, 0.08+off, 0, global.brightRed, 0.5 + off);
		}
		if (contLight <= 0)
		{
			contLight = 20;
