...
rope1.vertexChangeData(last, x-100,y,,false)

for (var i=2; i<=_segments-1; i++)
{
	instBody[i] = instance_create_layer(x,y,"PauseHUE_Sub",oSegmentsHitBox)
	rope1.vertexAttachObject(i, instBody[i],vertexAttachmentType.both);
	instBody[i].worm = inst;
}

plugged = false;
force = verletSystem1.addForceField(x-200, y-80, x+30, y+260,180, 0.4, false);
