...
function SpawnEnemyMiniAndFastBreach()
{
	if instance_exists(o_playerShip)
	{
		for (var i=0; i<1; i++)
		{	
			peta =  choose(o_enchufePETA,o_enchufePETAAll,o_enchufePETALaser,o_enchufePETAAmmo,
			o_enchufePETAAmmo,o_enchufePETAAmmo,o_enchufePETAAmmo,o_enchufePETAHP)
			if (petaCreated = false)
			{
				enemy = choose(o_enemyMini,o_enemyFast,o_enemyMini,o_enemyFast,o_enemyMini,o_enemyFast,o_enemyMini,o_enemyFast,o_enemyMini,o_enemyFast,peta)
			}
			else
			{
				enemy = choose(o_enemyMini,o_enemyFast)
			}
