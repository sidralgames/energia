...
function UIBossGarbanzo()
{
	if (room = Sala_SecondBoss) && instance_exists(o_BossMecha_Inside) && (global.BossGarbanzo_Awaken = true) // && !instance_exists(o_BreachBossBanner)
	{
		...
		if (o_BossMecha_Inside.hp >= 90)
		{
			draw_set_font(global.customFont16)
			draw_set_halign(fa_right)
			draw_set_color(global.brightRed)
			draw_text(__view_get( e__VW.XView, 0 )+334,__view_get( e__VW.YView, 0 )+4,string(round(o_BossMecha_Inside.hp)))
			draw_set_color(c_white)
			draw_text(__view_get( e__VW.XView, 0 )+335,__view_get( e__VW.YView, 0 )+3,string(round(o_BossMecha_Inside.hp)))
		}
		else
		{
			draw_set_font(global.customFont16)
