...
function DrawUpgradeNamesGameOver()
{
	for (var i = 0; i<global.numberOfUpgradesPlayer; i++)
	{
		...
		if (upgrades.times > 0) && (upgrades.inStat == false)
		{
			...
			if (upgrades.times = 1)
			{
				...
				draw_text(__view_get( e__VW.XView, 0 )+47,__view_get( e__VW.YView, 0 )+200+timesToDraw*15, upgrades.nameUpgrade)
			}
			else
			{
				draw_set_halign(fa_left)
				...
				draw_set_color(global.yellow)
				draw_text(__view_get( e__VW.XView, 0 )+47,__view_get( e__VW.YView, 0 )+200+timesToDraw*15, string(upgrades.nameUpgrade) +" x" + string(upgrades.times))
			}
		}
	}
}