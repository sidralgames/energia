...
function BestiaryDrawUnknown()
{
	...
	draw_text(__view_get( e__VW.XView, 0 )-1+initialXInterrog,__view_get( e__VW.YView, 0 )+InitialYToKill+1,"Killed: " + string(enemy.killsNum) + "/" + string(enemy.killsToUnlock));
	draw_set_color(c_white);
	draw_text(__view_get( e__VW.XView, 0 )+initialXInterrog,__view_get( e__VW.YView, 0 )+InitialYToKill,"Killed: " + string(enemy.killsNum) + "/" + string(enemy.killsToUnlock));
	
	for (var i=0; i<2; i++)
	{
		draw_set_font(global.customFont16);
		draw_set_color(global.brightRed);
		draw_text(__view_get( e__VW.XView, 0 )-1+initialXText,__view_get( e__VW.YView, 0 )+ (InitialYName+1) + (i*(160 +aliasOff)),"??? ?? ???");
		draw_set_color(c_white)
